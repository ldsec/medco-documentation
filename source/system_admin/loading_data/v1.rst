...
The generation of the dummies will take some time (around 5min with the default data) so please wait for the container to finish. 
The output files are placed inside ``~/medco-loader/data/scripts/import-tool/data/output``. **You only need to replace the old** ``dummy_to_patient.csv`` **with the new one that was generated (ignore all the others files).**
